...
float pattern(vec2 st, vec2 v, float t) {
    vec2 p = floor(st+v);
    return step(t, random(100.+p*.000001)+random(p.x)*0.220 );
}
...
void main() {
   ...
   vec2 grid = vec2(50.0,100.);
    ...
    vel *= vec2(0.0,1.) * random(0.+ipos.y); // direction
    ...
    color.r = pattern(st+offset,vel,0.3);
    color.g = pattern(st,vel,0.3);
    color.b = pattern(st-offset,vel,0.3);
