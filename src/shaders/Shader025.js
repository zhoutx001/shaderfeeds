...
export default {
  ...
  void main (void) {
      vec2 st = gl_FragCoord.xy/u_resolution.xy-vec2(-0.160,-0.090);
    float pos;
    pos = u_time;
    ...
    // st=fract(st);
     ...
     //pattern += circle(st- vec2(-0.030,0.430),0.432);
      // pattern += circle(st- vec2(0.550,0.350),0.120);
      pattern +=circle(st+ vec2(0.550),0.016);
  ...
  //                          0.010);
    ...
    //pattern+=capsule(st, vec2(0.890,0.680), vec2(0.05, 0.05), 0.706);
      pattern+=box(st,vec2(0.2));
      //pattern+=cross(st,.4);
    
      
      //pattern+=capsule(st - vec2(-0.890,0.060), vec2(-0.05, -0.05), vec2(0.05, 0.05), 1.098);
