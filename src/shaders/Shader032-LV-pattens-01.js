...
// 	pattern=smoothedge(pattern);
    ...
    //rotate
    //st = rotate2D(st,-PI*u_time*-0.070);
      
      
    //half capsule
    //pattern+=box(st-vec2(-0.150,0.210),vec2(.2));
    // pattern+=circle(st,0.075);

    if(stOrigin.x < 2./3. && stOrigin.x > 1./3.){          

        //circle
    	pattern=circle(st-vec2(0.010,-0.130),0.170);
        
		
    }else if(stOrigin.y < 2./3. && stOrigin.y > 1./3.){
