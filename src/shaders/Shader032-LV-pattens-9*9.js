...
export default {
  ...
  void main (void) {
    ...
    float pattern = 0.0;
  	...
  	st = tile(stOrigin,tileNum);  
     
    //strech
    //st = tile(st,-1.552*st.x);

	if(mod(floor(stOrigin.x *tileNum),2.)!= mod(floor(stOrigin.y *tileNum),2.)){          
        pattern+=circle(st-vec2(0.010,-0.110),0.170);
    }
	else if(mod(floor(stOrigin.y *tileNum),2.) == mod(floor(stOrigin.x *tileNum),2.)&&mod(floor(stOrigin.x *tileNum),2.)==0.){
    	st = rotate2D(st, -PI*0.25); 
        pattern+=box(st,vec2(.25));
    }else{
        st-=vec2(.5);
        //size of the clover
        st = rotateTile(0.400*st);        
        //clover
    ...
    st = rotate2D(st, PI*0.25); 
    pattern+=cross(st-vec2(0.500,-0.50),0.3);
